...
section \ref{sec:inv}, it can be seen that $\mathcal{L}_{\theta} =
...
Let $\mathcal{D}={(x_{i}, y_{i}): i = 1 \cdots n_{o}}$ be a set of
...
can now be cast as the following constrained $L_2$ regularized least squares problem.

\begin{align}\label{eq:surrogate}
   min_{w,e,\epsilon} \ \mathcal{J}(w,e,\epsilon;\theta) &= \frac{1}{2}
                                                    w^{T}w +
                                                    \frac{1}{2\gamma_{o}}
                                                    \sum_{k =
                                                    1}^{n_{o}}{e^{2}_{k}}
                                                    +
                                                    \frac{1}{2\gamma_{c}}
                                                    \sum_{k = 1}^{n_{c}}{\epsilon^{2}_{k}} \\
                                                & s.t \\
  y_{i} & = w^{T}\varphi(x_{i}) + b + e_{i}, \ \ i = 1 \cdots n_{o} \\
  q_{i} & = w^{T}\psi_{\theta}(x_{i}) + \epsilon_{i}, \ \ i = 1 \cdots n_{c}
\end{align}
...
\begin{align*}\label{eq:lag}
      \mathfrak{L}(w,e,\epsilon, \alpha_{1 \cdots k}, \beta_{1 \cdots
    k};\theta; \gamma_{o}; \gamma_{c}) &= \frac{1}{2}
                                                    w^{T}w +
                                                    \frac{1}{2\gamma_{o}}
                                                    \sum_{k =
                                                    1}^{n_{o}}{e^{2}_{k}}
                                                    ...
                                                    \frac{1}{2\gamma_{c}}
                                                    \sum_{k =
                                                      1}^{n_{c}}{\epsilon^{2}_{k}} \\
                                                   & + \sum_{k =
                                                      1}^{n_{o}}{\alpha_{k}(y_{k}
                                                      -
                                                      w^{T}\varphi(x_{k})
                                                      + b + e_{k})} \\
                                                   & + \sum_{k =
                                                      1}^{n_{c}}{\beta_{k}(q_{j}-
                                                     w^{T}\psi_{\theta}(x_{j}) + \epsilon_{j})} 
\end{align*}

Given fixed values for $\gamma_{o}$, $\gamma_{c}$ and PDE parameters
$\theta$, the equation above expresses the Lagrangian of system
\ref{eq:surrogate}. The quantities $\alpha_{1 \cdots n_o}$ and
$\beta_{1 \cdots n_c}$ are the \emph{Lagrange multipliers}
introduced for equality constraints of the system. Applying the
\emph{Karush-Kuhn-Tucker}(KKT) conditions the solution can be computed
in the so called \emph{dual space}.
